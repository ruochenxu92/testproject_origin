...
class GroupSpider(CrawlSpider):
    ...
    def parse_group_home_page(self, response):
        ...
        item = DoubanItem()
        #get group name
        item['groupName'] = response.xpath('//h1/text()').re("^\s+(.*)\s+$")[0]
        #get group id
        ...
        members_text = response.xpath('//a[contains(@href, "%s")]/text()' % members_url).re("\((\d+)\)")
        item['totalNumber'] = members_text[0]
        #get relative groups
        item['RelativeGroups'] = []
        groups = response.xpath('//div[contains(@class, "group-list-item")]')
        for group in groups:
